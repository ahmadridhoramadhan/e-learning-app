{{-- question card --}}
<div class="border-b-2 border-gray-300 mb-3 pb-3 last:border-b-0">
    ...
    <ul class="pl-1 flex flex-col gap-1 list-disc ml-5">
        @foreach ($question->question->answers as $answer)
            <li
                class="break-all {{ $answer->id == $question->answer_id ? ' text-red-700 font-semibold ' : '' }} {{ $answer->id == $question->question->answer_id ? ' !text-green-700 font-semibold ' : '' }}">
                {!! $answer->answer !!}
            </li>
        @endforeach
    </ul>
