<div class="relative" x-data="{ isOpen: false }">
    <div class="border-2 transition-all flex items-center px-3 py-2 gap-3 relative z-10 rounded shadow-md"
        :class="isOpen && 'border-cyan-700 bg-cyan-100 w-fit ml-3'" @click="isOpen=!isOpen">
        <div><img src="" alt="teacher foto profile" class="rounded-full aspect-square"
                :class="isOpen ? 'w-6' : 'w-10'"></div>
        <div class="w-32 truncate">{{ $teacher->name }}</div>
    </div>

    <div class="border-2 border-black pt-8 pb-3 grid grid-cols-2 px-6 gap-5 rounded-md relative -top-5 w-full"
        @click.away="isOpen = false" x-show="isOpen">

        @forelse ($teacher->rooms as $room)
            @if ($room->is_active)
